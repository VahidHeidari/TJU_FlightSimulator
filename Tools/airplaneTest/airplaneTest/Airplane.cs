    ...
    class Sections
    {
        ...
        public Sections(string Name ,string FilePath,string TexturePath)
        {
            ...
            if (!m_Name.StartsWith("GLASS", StringComparison.CurrentCultureIgnoreCase))
                m_model = new IndexedNormal(FilePath, TexturePath);
            else
                m_model = new Glass(FilePath, TexturePath);
        }
        public Sections(string Name, string FilePath, Texture texture)
        {
            m_Name = Name;
            if (!m_Name.StartsWith("GLASS", StringComparison.CurrentCultureIgnoreCase))
                m_model = new IndexedNormal(FilePath, texture);
            else
                m_model = new Glass(FilePath, texture);
        }
